...
- Generate: `/superwriter <voice>: <brief>` - e.g. `/superwriter corporate: a note declining a meeting invite`
- Transform: [paste text] `/superwriter <voice>` - e.g. `/superwriter journalistic`
- Transform to a length: add `to <N> words` - e.g. `/superwriter journalistic to 40 words`
- Proofread: [paste text] `/superwriter proofread`
- Other commands: `analyze`, `analyze as <name>`, `blend <a> + <b>`, `<voice> --example`
